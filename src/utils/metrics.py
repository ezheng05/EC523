...
# clinical thresholds for binary AUC
# cesd>=10, stais>=40, pss>=14, ucla>=25, brs<=3.0 (lower = worse)
THRESHOLDS = [10.0, 40.0, 14.0, 25.0, 3.0]
...
def compute_regression_metrics(y_pred, y_true):
    ...
    for i, name in enumerate(TARGET_NAMES):
        p = y_pred[:, i].numpy() if hasattr(y_pred, 'numpy') else np.asarray(y_pred[:, i])
        t = y_true[:, i].numpy() if hasattr(y_true, 'numpy') else np.asarray(y_true[:, i])
...
def compute_auc_metrics(y_pred, y_true, thresholds=None):
    ...
    for i, (name, thresh) in enumerate(zip(TARGET_NAMES, thresholds)):
        if thresh is None:
            ...
            continue
        p = y_pred[:, i].numpy() if hasattr(y_pred, 'numpy') else np.asarray(y_pred[:, i])
        t_arr = y_true[:, i].numpy() if hasattr(y_true, 'numpy') else np.asarray(y_true[:, i])
        if name in _LOWER_IS_WORSE:
            t_bin = (t_arr <= thresh).astype(int)
            score = -p  # flip so higher score = more likely positive
        else:
            t_bin = (t_arr >= thresh).astype(int)
            score = p
        if t_bin.sum() == 0 or t_bin.sum() == len(t_bin):
            ...
            continue
        results[name] = {
            "auc_roc": float(roc_auc_score(t_bin, score)),
            "auc_pr":  float(average_precision_score(t_bin, score)),
        }
    return results


def print_metrics(reg, auc):
    header = f"{'target':<14} {'rmse':>7} {'mae':>7} {'pearson_r':>10} {'r2':>7} {'auc_roc':>8} {'auc_pr':>7}"
    print(header)
    print("-" * len(header))
    for name in TARGET_NAMES:
        ...
        print(
            f"{name:<14} {r['rmse']:>7.3f} {r['mae']:>7.3f} {r['pearson_r']:>10.3f} "
